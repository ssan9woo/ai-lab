...
**1차 실험 종료 (2026-08-21)**: 목표했던 파이프라인을 끝까지 조립하고 실제로 동작시키는 데 성공. 결론·한계·Kane과의 비교는 `README.md` 참고. 애매한 표현 견고성(2차 실험, `IMPROVE.md`)은 이번엔 진행하지 않기로 함. 이 문서는 과정 기록으로 남겨두고, 정리된 요약은 README.md가 대표 문서.
...
내부적으로 `dart run bin/verify.dart`를 실행하는 래퍼 스크립트. 자연어 지시를 파싱하고, iOS 시뮬레이터에서 실제로 실행한 뒤 터미널 요약과 `evidence/<timestamp>/` 폴더를 남긴다.
...
- [x] DSL을 실제로 실행하는 Dart 러너 작성 — `runActionPlan()`(`integration_test/support/runner.dart`, 테스트 트리 소속)이 액션 타입별로 `tester.tap()`/`tester.enterText()`/위젯 상태 확인으로 매핑, 스텝별 pass/fail 격리 기록. iOS 시뮬레이터에서 로그인+체크박스 토글 플랜, 존재하지 않는 타겟 실패 플랜 모두 검증
- [x] 결과 출력 — `bin/verify.dart`(host 오케스트레이터)가 자연어 → NL 파싱(host) → 생성된 integration test로 시뮬레이터 실행(device) → 마커 기반 stdout 파싱 → 터미널 요약(✓/✗, 소요시간) + `evidence/<timestamp>/`(instruction.txt, plan.json, result.json) 저장. 스크린샷은 이 Flutter 버전/실행 조합에서 안정적으로 파일화되지 않아 제외
- [x] 로그인 플로우, 각각 다른 문장 표현(같은 의도, 다른 어휘)으로 두 번 실행 — `plan.json` diff 결과 완전히 동일한 DSL로 수렴 확인
- [x] 존재하지 않는 위젯을 요구하는 지시로 fail 케이스 확인 — 예외로 죽지 않고 `result.json`에 `passed: false` + 에러 메시지로 정상 기록됨, 종료 코드 1
